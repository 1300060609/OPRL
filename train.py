...
class TrainOPRLConfig:
    ...
    @classmethod
    def from_dict(cls, configures: Dict) -> 'TrainOPRLConfig':
...
class TrainOPRL:
    ...
    def __init__(self, configures: Dict) -> None:
        train_conf = TrainOPRLConfig.from_dict(configures)
        ...
        self.model=OPRL(configures).to(self.device)
        # self.model=torch.nn.DataParallel(OPRL(configures)).module.fc()
        # self.model.to(self.device)

    def _get_edges(self,input_dir):
        Eca=[]
        with open(os.path.join(input_dir,'cell_context.txt'),'r') as f1:
            for line in f1:
                l=line.strip().split(',')
                Eca.append([l[0],l[1],eval(l[2])])
        Ega=[]
        if self.prior_weight:
            with open(os.path.join(input_dir,'gene_context.txt'),'r') as f2:
                for line in f2:
                    l=line.strip().split(',')
    ...
    def _get_vocab(self):
        ...
        for tri in total_edges:
            ...
            if con not in vocab:
                vocab[con]=i
                i+=1
        return vocab  # 词汇表:dict,每个单词对应一个索引值 word:index
    ...
    def run(self):
        ...
        context_w_eca = self._get_contextgene_value(self.Eca)  # 获取每个context gene概率，这里以频率的3/4次方作为构建依据
        contexts_eca,context_pool_eca=self._get_context_pool(context_w_eca)

        optimizer = torch.optim.SGD(OPRL.parameters(self.model), lr=self.learning_rate)
        ...
        while loop <= self.T:
            # sys.stdout.write('\r当前进度 第%d次更新 剩余%d次' % (loop, self.T-loop))
            # sys.stdout.flush()
            start_training = timeit.default_timer()
            # sample a positive edge from Eca and K negative edges form the noise distribution Pca and update
            # the cells's representations and the context-genes' representations
            eca = self.Eca[edge_pool_eca.pop()]  # 目标二元组
            ...
            # the genes's representations and the context-genes' representations
            if self.prior_weight:
                ega = self.Ega[edge_pool_ega.pop()]  # 目标二元组
                ega_weight=ega[2]
                batch_pos_ega = torch.tensor([[self.vocab[ega[0]], self.vocab[ega[1]]]],device=self.device)
                self._update_parameters(optimizer, batch_pos_ega, None,self.prior_weight*ega_weight/np.abs(ega_weight))

            stop_training = timeit.default_timer()
            sys.stdout.write('\r当前进度 第%d次更新 剩余%d次 单次消耗时间%f秒 预计还需%f小时' % (loop, self.T-loop,stop_training-start_training,(stop_training-init_time)/loop*(self.T-loop)/3600))
            sys.stdout.flush()
            loop += 1
    ...
    def _get_contextgene_value(self, edges):
        res = {}
        for tri in edges:
            con = tri[1]
            val = np.abs(tri[2])
    ...
    def random_pick(self,item_ps: List):
        """
            :param item_ps:like [['gene1', 0.1065], ['gene2', 0.3164], ['gene3', 0.226], ['gene4', 0.3511]]
        """
    ...
    def _get_neg_samples(self, pos_index, contexts, context_pool):
        """
                ...
                :param pos_index: 目标样本在词汇表中的索引
                :param context_p: list of [context: probability]
                :return: 负例样本在词汇表中的索引的数组
                """
        neg_samples = []
        while len(neg_samples) < self.K:
            neg_sample_index = contexts[context_pool.pop()]
            if neg_sample_index == pos_index:  # 负采样样本不能与目标样本相同
                continue
    ...
    def _update_parameters(self, optimizer, batch_pos, batch_neg, weight):
        ...
        # print(batch_pos)
        optimizer.zero_grad()
        loss = OPRL.forward(self.model, batch_pos, batch_neg)*weight
...
if __name__ == '__main__':
    Eca = [('cell1', 'gene1', 1), ('cell1', 'gene2', 2), ('cell2', 'gene2', 3), ('cell3', 'gene3', 4)]
    Ega = [['aa', 'gene1', 1.1], ['bb', 'gene2', 2.3], ['sdf', 'gene2', 2.4], ['fds', 'gene3', 3],
           ['dfff', 'gene4', 5.4]]
